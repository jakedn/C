...
void insert(int dataToAdd,int *numberOfNodes)
{
    ...
    (*numberOfNodes)++;    //we need to add one from what numberofnodes points to not numberofnodes itself
}

//didnt have return comment so i added.
/**
...
//delete is keyword in C cant be name of function so renamed to deleteNode
{
    ...
    while(temp != NULL)
    {
        if(temp->data == dataToDelete)
        {
            ...
            {
                head=temp->next;
                free(temp->data);   //making sure there are no leaks
                free(temp);
            ...
            (*numberOfNodes)--;     //we need to subtract one from what numberofnodes points to not numberofnodes itself
            return 1;
